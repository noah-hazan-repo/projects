...
Torah_formatted = Torah_raw.split(' ')


def numerical_value(hebrew_letter):
    return number_conversion[alphabet_values[hebrew_letter]]

def find_by_number(number):
    matches = []
    for word in Torah_formatted:
        value = 0
        for letter in word:
            if letter in 'אבגדהוזחטיכךלמםנןסעפףצץקרשת':
                letter_val = numerical_value(letter)
                if letter_val is None:
                    continue
                value = value + letter_val
        if value == number:
            matches.append(word)
    ...
    for word in unique_matches:
        word = word.rstrip()
        clean_final_matches.append(word)
    return clean_final_matches
...
def find_by_word(word):
    ...
    for letter in word:
        if letter in 'אבגדהוזחטיכךלמםנןסעפףצץקרשת':
            letter_val = numerical_value(letter)
            value = value + letter_val
    print(f'The word {word} has the gematria of {value}')
    time.sleep(1)
    print(f'Finding other words with the value of: {value}')
    time.sleep(2)
    print('Still looking...')
    time.sleep(2)
    print('Found all words that match that value. See below.')
    time.sleep(1)
    
    other_words = find_by_number(value)
...
#### testing below
## generate full gematria dictionary
## 
## Torah_Gematrias = {}
## for word in split:
##     value = 0
...
##             letter_val = numerical_value(letter)
##             value = value + letter_val
##     Torah_Gematrias[word] = value
##     
##     
##     
## def find_gematrias_that_match(number):
##     print([k for k,v in Torah_Gematrias.items() if v == number])
